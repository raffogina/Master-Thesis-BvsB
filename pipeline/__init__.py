...
The former single-file script is split into one module per pipeline step.
Each step reads the previous step's output file(s) from disk, does its own
work, and writes its own output file(s) — so a failure in one step stops
